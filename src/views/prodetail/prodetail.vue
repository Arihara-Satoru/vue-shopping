...
<script>
import { getProComments, getProCommentsCount, getProductDetail } from '@/api/product'
...
export default {
  ...
  methods: {
    ...
    async getProComments () {
      const res = await getProComments(this.$route.query.id, 3)
    ...
    async getProCommentsCount () {
      const res = await getProCommentsCount(this.$route.query.id)
